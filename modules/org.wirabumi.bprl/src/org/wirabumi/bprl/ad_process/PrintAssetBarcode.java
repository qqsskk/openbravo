...
public class PrintAssetBarcode extends DalBaseProcess {
	...
	public void printAssetRawText(String documentID, String printaddress, String printport) throws Exception {
		...
		for (int i=0; i<assetKeyList.size(); i++){
			...
			//asset kanan
			sb.append(buildAssetCodeToPrintKanan(assetKeyList.get(i), assetKeyList.get(i)));
			...
			//asset kiri
			sb.append(buildAssetCodeToPrintKiri(assetKeyList.get(i), assetKeyList.get(i)));
		...
		RawPrintUtility.sendToPrint(sb.toString(), printaddress, printport);
	...
	protected void doExecute(ProcessBundle bundle) throws Exception {
		...
		String printaddress = (String) bundle.getParams().get("printeraddress");
		if (printaddress==null || printaddress.isEmpty())
			throw new OBException("print address is null or empty");
		
		String printport = (String) bundle.getParams().get("printerport");
		if (printaddress==null || printaddress.isEmpty())
			throw new OBException("print port is null or empty");
		
		
		try {
			//print asset to barcode printer
			printAssetRawText(documentID, printaddress, printport);
		} catch (Exception e) {
